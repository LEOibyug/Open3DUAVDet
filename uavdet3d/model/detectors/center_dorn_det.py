...
class CenterDORNDet(DetectorTemplate):
    ...
    def post_processing(self, batch_dict):
        ...
        center_dis_bin = batch_dict['pred_center_dict']['center_dis_bin']
        center_dis_res = batch_dict['pred_center_dict']['center_dis_res']
        dim = batch_dict['pred_center_dict']['dim']
        rot = batch_dict['pred_center_dict']['rot']

        # hm = batch_dict['hm'] #
        # center_res = batch_dict['center_res']
        # center_dis_bin = batch_dict['center_dis_bin']
        # center_dis_res = batch_dict['center_dis_res']
        # dim = batch_dict['dim']
        # rot = batch_dict['rot']
        ...
        center_dis_bin = reshape_t(center_dis_bin, batch_size)
        center_dis_res = reshape_t(center_dis_res, batch_size)
        dim = reshape_t(dim, batch_size)
        rot = reshape_t(rot, batch_size)

        max_dis = self.dataset.dataset_cfg.MAX_DIS
        for batch_id in range(batch_size):
            ...
            this_center_dis_bin = center_dis_bin[batch_id]

            this_center_dis_res = center_dis_res[batch_id]

            _, depth_num, _, _ = this_center_dis_bin.shape
