...
class MAV6DDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, training, root_path, logger):
        super(MAV6DDataset, self).__init__(dataset_cfg=dataset_cfg, training=training, root_path=root_path, logger=logger)
        ...
        self.sample_scene_list = [x.strip() for x in open(split_dir).readlines()]
    ...
    def set_split(self, split):
        super(MAV6DDataset, self).__init__(dataset_cfg=self.dataset_cfg, training=self.training, root_path=self.root_path, logger=self.logger)
    ...
    def include_MAV6D_data(self, mode):
        ...
        for i, scene_name in enumerate(self.sample_scene_list) :

            all_seq = os.listdir(os.path.join(root_path, self.im_path_name, scene_name))


            for seq_name in all_seq:

                all_frame = os.listdir(os.path.join(root_path, self.im_path_name, scene_name, seq_name))

                num = np.array(list(map(lambda x:float(x.split('.')[0]), all_frame)))

                sor = np.argsort(num)

                for k_i in range(0, len(all_frame), self.dataset_cfg.SAMPLED_INTERVAL[mode]) :
                    frame_name = all_frame[sor[k_i]]

                    each_im_path = os.path.join(root_path, self.im_path_name,scene_name, seq_name, frame_name)
                    each_label_path = os.path.join(root_path, self.label_path_name,scene_name, seq_name, frame_name.replace('jpg', 'txt'))

                    data_info = {'im_path': each_im_path, 'label_path':each_label_path, 'scene_id':scene_name, 'seq_id':seq_name, 'frame_id': frame_name}
    ...
    def evaluation(self, annos, metric_root_path):
        ...
        orin_error, pos_error = eval_6d_pose(annos)
        orin_error = orin_error/np.pi*180
        ...
        return_str = 'key_point_error: '+'orin_error： '+str(orin_error.mean()) + '\n'\
