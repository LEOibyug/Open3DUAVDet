...
class DataPreProcessor():
        ...
        for im_id in range(im_num):
            ...
            for ob_id in range(obj_num):
                ...
                for k_i, center in enumerate(ob_pts):
                    ...
                    this_residual_x[k_i], this_residual_y[k_i] = draw_res_to_heatmap(this_residual_x[k_i],this_residual_y[k_i], center)
    ...
    def convert_box9d_to_centermap_DORN(self, data_dict=None, config = None):
        if data_dict is None:
            return partial(self.convert_box9d_to_centermap_DORN, config=config)

        center_rad = self.dataset_cfg.CENTER_RAD
        class_name_config = self.dataset_cfg.CLASS_NAMES
        im_num = self.dataset_cfg.IM_NUM

        max_dis = self.dataset_cfg.MAX_DIS

        depth_num = config.DEPTH_NUM

        depth_interval = max_dis/depth_num
        ...
        gt_hm, gt_center_res, gt_center_dis_bin, gt_center_dis_res, gt_dim, gt_rot = center_point_encoder(gt_box9d_with_cls,
                                                                                                           intrinsic,
                                                                                                           extrinsic,
                                                                                                           distortion,
                                                                                                           new_im_size[0],
        ...
        # np.save('gt_hm', gt_hm)
        # np.save('im', data_dict['image'])
        # input()

        data_dict['hm'] = gt_hm
        data_dict['center_res'] = gt_center_res

        data_dict['center_dis_bin'] = gt_center_dis_bin
        data_dict['center_dis_res'] = gt_center_dis_res/depth_interval
    ...
    def convert_box9d_to_centermap_DORN_w2d(self, data_dict=None, config = None):
        if data_dict is None:
            return partial(self.convert_box9d_to_centermap_DORN_w2d, config=config)
        ...
        gt_hm, gt_center_res, gt_center_dis_bin, gt_center_dis_res, gt_dim, gt_dim2d, gt_rot = center_point_encoder(gt_box9d_with_cls,
                                                                                                                   this_boxes_2d,
                                                                                                                   intrinsic,
                                                                                                                   extrinsic,
                                                                                                                   distortion,
        ...
        data_dict['dim'] = gt_dim/self.dataset_cfg.MAX_SIZE
        data_dict['dim2d'] = gt_dim2d / dim2d_max
        data_dict['rot'] = gt_rot
    ...
    def convert_box9d_to_centermap_DORN_depth(self, data_dict=None, config = None):
        if data_dict is None:
            return partial(self.convert_box9d_to_centermap_DORN_depth, config=config)
        ...
        gt_hm, gt_center_res, gt_center_dep, gt_center_dis_bin, gt_center_dis_res, gt_dim, gt_rot = center_point_encoder(gt_box9d_with_cls,
                                                                                                           intrinsic,
                                                                                                           extrinsic,
                                                                                                           distortion,
                                                                                                           new_im_size[0],
        ...
        data_dict['center_dis_bin'] = gt_center_dis_bin
        data_dict['center_dep'] = gt_center_dep/max_dis
        data_dict['center_dis_res'] = gt_center_dis_res/depth_interval
    ...
    def convert_box9d_to_centermap_DORN_cs(self, data_dict=None, config = None):
        if data_dict is None:
            return partial(self.convert_box9d_to_centermap_DORN_cs, config=config)
        ...
        max_dis = config.NormMaxDis

        max_size = config.NormMaxSize
        ...
        data_dict['center_dis_bin'] = gt_center_dis_bin
        data_dict['center_dis_res'] = gt_center_dis_res

        data_dict['dim'] = gt_dim
        data_dict['rot'] = gt_rot
    ...
    def filter_box_outside(self, data_dict=None, config = None):
        ...
        for i in range(im_num):
            ...
            if len(gt_box9d)>0:
                ...
                corners_2d, _ = cv2.projectPoints(gt_pts, extrinsic[i, :3, :3], extrinsic[i, :3, 3], intrinsic[i], distortion[i])
