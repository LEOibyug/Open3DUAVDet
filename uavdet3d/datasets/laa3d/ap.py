...
def calculate_center_distance(box1: np.ndarray, box2: np.ndarray) -> float:
    ...
    Args:
        box1, box2: [x, y, z, l, w, h, roll, yaw, pitch]

    Returns:
        Euclidean distance between centers
    """
    center1 = box1[:3]  # [x, y, z]
    center2 = box2[:3]  # [x, y, z]
...
def match_boxes_by_distance(pred_boxes: np.ndarray,
    ...
    Args:
        pred_boxes: (N, 9) array of predicted boxes [x,y,z,l,w,h,roll,yaw,pitch]
        gt_boxes: (M, 9) array of ground truth boxes
...
def calculate_ap_for_recall_points(precisions: np.ndarray,
                                   recalls: np.ndarray,
                                   recall_points: np.ndarray) -> float:
    """
...
def calculate_object_detection_3dap(pred_boxes: np.ndarray,
                           ...
                           detection_distances: List[float] = [100]) -> List[float]:
    """
    Calculate Average Precision for 3D object detection with center distance matching.

    Args:
        pred_boxes: (N, 9) predicted boxes [x,y,z,l,w,h,roll,yaw,pitch]
    ...
    for max_distance in detection_distances:
        ...
        for match_threshold in matching_thresholds:
            ...
            for frame_id in frame_ids:
                ...
                frame_scores = pred_scores[pred_indices]
                pred_distances = np.linalg.norm(frame_pred_boxes[:, :3], axis=1)
                ...
                gt_distances = np.linalg.norm(frame_gt_boxes[:, :3], axis=1)
