...
def eval_box6d_error(annos, max_dis = 8):
    ...
    for i, each_anno in enumerate(annos) :
        ...
        gt_box9d, pred_boxes9d = match_gt(gt_box9d, pred_boxes9d, gt_box2d, pred_box2d)
        ...
        x_error = np.abs(pred_xyz[:, 0] - gt_xyz[:, 0])
        y_error = np.abs(pred_xyz[:, 1] - gt_xyz[:, 1])
        z_error = np.abs(pred_xyz[:, 2] - gt_xyz[:, 2])
    ...
    if len(all_orin_error)>0 and len(all_pos_error)>0 and len(all_size_error)>0:
        ...
        all_x_error = np.concatenate(all_x_error)
        all_y_error = np.concatenate(all_y_error)
        all_z_error = np.concatenate(all_z_error)
    else:
        all_orin_error=np.array([0])
        all_pos_error=np.array([0])
        all_size_error = np.array([0])
        all_x_error = np.array([0])
        all_y_error = np.array([0])
        all_z_error = np.array([0])

    return all_orin_error, all_pos_error, all_size_error, all_x_error, all_y_error, all_z_error
...
def eval_6d_pose(annos, max_dis = 100):
    ...
    orin_error, pos_error, size_error, all_x_error, all_y_error, all_z_error = eval_box6d_error(annos, max_dis)

    return orin_error, pos_error, size_error, all_x_error, all_y_error, all_z_error
