...
def box9d_to_2d(boxes9d, img_width=1920., img_height=1080., intrinsic_mat=None, extrinsic_mat=None,
...
def box9d_to_2d_opencv(boxes9d, img_width=1280., img_height=720., intrinsic_mat=None):
    """
    使用 OpenCV 将 9D box (x,y,z,l,w,h,rx,ry,rz) 投影到图像平面
    不考虑外参和畸变。
    每个 box 对应一个 3D 立方体，通过 cv2.projectPoints 投影到 2D。
    返回：
        boxes2d: (N, 4) -> [x1, y1, x2, y2]
        size: (N,) -> box 2D 尺寸（最小边）
    """
    ...
    # 遍历每个 9D box
    for box in boxes9d:
        x, y, z, l, w, h, rx, ry, rz = box[0], box[1], box[2], box[3], box[4], box[5], box[6], box[7], box[
            8]

        # 缩放角点
        corners = corners_local * np.array([l, w, h], dtype=np.float32)

        # 旋转
        rot_mat = R.from_euler('xyz', [rx, ry, rz], degrees=False).as_matrix().astype(np.float32)
        ...
        corners_2d, _ = cv2.projectPoints(corners, rvec, tvec, intrinsic_mat, None)
        corners_2d = corners_2d.reshape(-1, 2)

        # 限制范围
        corners_2d[:, 0] = np.clip(corners_2d[:, 0], 0, img_width - 1)
        corners_2d[:, 1] = np.clip(corners_2d[:, 1], 0, img_height - 1)
    ...
    return np.array(boxes2d, dtype=np.float32), np.array(size, dtype=np.float32)
...
def draw_box9d_on_image_with_info(boxes9d, image, info, img_width=1920., img_height=1080., color=(255, 0, 0),
    ...
    if distortion_matrix is None:
        distortion_matrix = np.zeros(5, dtype=np.float32)
    # info = [ float(x<0.2) for x in info]
...
class LAA3DReal_Det_Dataset(DatasetTemplate):
    def __init__(self, dataset_cfg, training, root_path, logger):
        super(LAA3DReal_Det_Dataset, self).__init__(dataset_cfg=dataset_cfg, training=training, root_path=root_path,
                                                    logger=logger)
    ...
    def __getitem__(self, item):
        ...
        info = copy.deepcopy(self.data_info[item])

        seq_id = info['seq_id']
        frame_id = info['frame_id']
        relative_im_path = os.path.join(self.root_path, self.split, 'images', seq_id, frame_id)
        ...
        if self.data_augmentor is not None and self.training:
            ...
            random_info = copy.deepcopy(self.data_info[random_frame_id])
            random_image = cv2.imread(os.path.join(self.root_path, self.split, 'images', random_info['seq_id'],
                                                   random_info['frame_id'])).astype(np.float32)
        ...
        image = cv2.resize(
            image,
            (self.raw_im_width, self.raw_im_hight),  # 目标尺寸 (width, height)
            interpolation=cv2.INTER_AREA  # 缩小推荐使用区域插值
        )

        intrinsic /= 1.5

        this_boxes_2d = this_boxes_2d.astype(np.float32)
        ...
        #
        # cv2.imwrite('image/' + seq_id + frame_id, image)
    ...
    def generate_prediction_dicts(self, batch_dict, output_path):
        ...
        for batch_id in range(batch_size):
            ...
            im_path = os.path.join(self.root_path, self.split, 'images', seq_id, frame_id)
            ...
            pred_box2d, _ = box9d_to_2d(pred_boxes9d, intrinsic_mat=intrinsic)
            gt_box2d, _ = box9d_to_2d(gt_box9d, intrinsic_mat=intrinsic)
            ...
            image = draw_box9d_on_image_with_info(pred_boxes9d, image, dis_error,
            ...
            image = draw_box9d_on_image(gt_box9d, image, # gt_box9d[:,0],
                                        img_width=raw_im_size[0],
                                        img_height=raw_im_size[1],
                                        color=(0, 0, 255),
                                        intrinsic_mat=intrinsic,
    ...
    def evaluation_all(self, annos, metric_root_path):
        ...
        laa_ads_fun = LAA3D_ADS_Metric(eval_config=self.dataset_cfg.LAA3D_ADS_METRIC, classes=self.class_name_dict,
